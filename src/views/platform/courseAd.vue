      ...
      <a-table :columns="columns" :data-source="list" rowKey="id" :pagination="pagination" :scroll="{ x: 600 }">
        ...
        <template slot="picture" slot-scope="text,record">
          <div style="width:150px;height:60px;">
            <img :src="record.picture" alt="error" style="width:150px;height:60px;">
          </div>
        </template>
        ...
        <template slot="action" slot-scope="text,record">
          <a style="margin-left:5px;color:#1890ff" @click="editProblem(record)" >编辑</a>
          <a style="margin-left:5px;color:red;" @click="deleteProblem(record.id)">删除</a>
        </template>
      </a-table>
    </div>
    <!--添加或者编辑轮播图 -->
    <a-modal v-model="visible" title="添加轮播图" ok-text="保存" cancel-text="取消" @ok="submitForm('form')" @cancel="cancelForm" option-label-prop="label">
      <a-form-model layout="vertical" :model="form" ref="form" :rules="rules" :validateOnRuleChange="true">
        <a-form-model-item label="轮播图" prop="picture">
          <uploadImage :uploadOption="uploadOption" @updateFilesList="updateFilesList" :picture="form.picture"></uploadImage>
        </a-form-model-item>
        <a-form-model-item label="标题" prop="title">
          <a-input v-model="form.title" placeholder="请输入少于50字符文字，不填写默认为“未命名”" />
        </a-form-model-item>
        <a-form-model-item label="跳转地址" prop="address">
           <a-input v-model="form.address" placeholder="请输入http://或者https://开头链接" />
        </a-form-model-item>
        ...
        <a-form-model-item label="展示时间" prop="rangeTime">
          <a-space direction="horizontal">
            <a-config-provider :locale="zhCN">
              <a-range-picker :disabled-date="disabledDate" v-model="form.rangeTime" :showTime="{ defaultValue: moment('00:00:00', 'HH:mm:ss') 
...
import { getCourseList,addCourseList,updateCourse,delCourse } from '@/api/platform/course'
export default {
  components:{
    uploadImage
  },
  data() {
    ...
    return {
      columns: [
        { title: " 标题", dataIndex:"title",key:"title", ellipsis: true, width: 150 },
        { title: "轮播图", scopedSlots: { customRender: 'picture' }, ellipsis: true, width: 150 },
        { title: "跳转地址", scopedSlots: { customRender: 'address' }, ellipsis: true, width: 200 },
        { title: "展示时间", scopedSlots: { customRender: 'rangeTime' }, ellipsis: true, width: 180 },
        { title: "排序", dataIndex: "sort", key: "sort", ellipsis: true, width: 85 },
        { title: "状态", scopedSlots: { customRender: 'enabled' }, ellipsis: true, width: 85 },
        {
          title: "操作",
          fixed: "right",
          width: 150,
      ...
      rules:{
         ...
         picture:[{ required:true,message:"请上传轮播图",trigger:"change"}],
        ...
        onOk() {
          delCourse({ id}).then(res => {
            if (res.code === 5200) {
              ...
              _this.pagination.current = (total - 1) % pageSize === 0 && _this.list.length === 1 ? current - 1 : current
              _this.getList()
            }
          })
        },
