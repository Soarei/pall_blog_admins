<template>
  ...
  <div class="app-container">
    <div class="search">
      ...
      <a-table :columns="columns" :data-source="list"  rowKey="catgory_id" :pagination="pagination">
          <template slot="catgory_icon" slot-scope="text,record">
            <img :src="record.catgory_icon" alt="error" srcset="" style="width:50px;height:50px;">
          </template>
          <template slot="action" slot-scope="text,record">
            <a style="margin-left:5px;" @click="editCloumn(record)">编辑</a>
            <a style="margin-left:5px;color:red;" @click="deleteCloumn(record)">删除</a>
          </template>
      </a-table>
      <!-- 添加分类 -->
      <a-modal v-model="dialogVisible" title="添加分类" ok-text="确认" cancel-text="取消" @ok="submitForm('form')" @cancel="cancelForm" option-label-prop="label">
      ...
      <a-form-model layout="vertical" :model="form"  ref="form" :rules="rules" :validateOnRuleChange="true">
        ...
        <a-form-model-item label="分类图标" prop="catgory_icon">
          ...
          <a-upload name="file" 
            ...
            @change="uploadChange"
            >
            <img v-if="form.catgory_icon" :src="form.catgory_icon" alt="avatar" style="width:80px;height:80px;" />
            <div v-else>
...
<script>
import { getCategoryList,addCategory,delCategory,editCategory } from "@/api/category";
...
export default {
  ...
  methods: {
    ...
    deleteCloumn(record){
      ...
      this.$antconfirm({
        title: '确认删除问题分类？',
        content: '删除问题分类后不能恢复，相关分类问题自动变为空分类，且不再商城展示，是否继续删除操作。',
        okText: '删除',
        cancelText: '取消',
        onOk() {
          delCategory({ catgory_id }).then(res => {
            if (res.code === 5200) {
              ...
              _this.pagination.current = (total-1)%pageSize === 0 ? current-1 : current
              _this.getList()
            }
          })
        },
