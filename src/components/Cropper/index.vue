<template>
  ...
  <div style="display:flex;justify-content:flex-start;">
    <div v-if="thumbId==='goodsThumb_id'" :id="'box'+thumbId" class="cropper-preview">
      <template v-for="(item,i) in allThumb.thumbImg">
        <img v-if="item" :src="item" :key='i' alt="img" style="margin-left:15px">
      </template>
    </div>
    <div v-if="thumbId==='adminUserIcon_id'" :id="'box'+thumbId" class="cropper-preview">
      <template v-for="(item,i) in allThumb.adminUserImg">
        <img v-if="item" :src="item" :key='i' alt="img" style="margin-left:15px">
      </template>
    </div>
    <div v-else-if="thumbId==='goodsCarousel_id'" :id="'box'+thumbId" class="cropper-preview">
      <draggable v-model="allThumb.carouserImg" @end="dragEnd">
        <template v-for="(item,i) in allThumb.carouserImg">
          <div v-if="item" class="cropper-close-box" :key='i'>
            <img :src="item" alt="img" style="width:100%;height:auto">
            <div class="close-tailoring close-goodsCarousel" @click="closeCarousel(item)">×</div>
          </div>
        </template>
      </draggable>
    </div>
    <div v-else-if="thumbId==='goodsDescribe_id'" :id="'box'+thumbId" class="cropper-preview">
      <draggable v-model="allThumb.describeImg" @end="dragDescEnd">
        <template v-for="(item,i) in allThumb.describeImg">
          <div v-if="item" :key='i' class="cropper-close-box">
            <div style="width:100%;height:105px;position:relative;overflow:hidden;">
              <img :src="item" alt="img" style="width:100%;height:auto;">
            </div>
            <div class="close-tailoring" @click="closeDescribe(item)">×</div>
          </div>
        </template>
      </draggable>
    </div>
    <div v-else :id="'box'+thumbId" class="cropper-preview">
      <img v-for="(item,i) in allThumb.combinationImg" :key='i' :src="item">
    </div>
    <div v-if="show" style="margin-left:15px;">
      <div class="l-btn" @click="showContain">{{ $t('table.selectImage') }}</div>
      <div v-if="showDelete" style="margin-left:15px;">
        <div class="l-btn" @click="showContain">Delete</div>
      </div>
    </div>
    <!--图片裁剪框 start-->
    <div v-show="showCropper" class="tailoring-container">
      ...
      <div class="tailoring-content">
        <div class="tailoring-content-one">
          <label title="$t('order.uploadImage')" for="chooseImg" class="l-btn choose-btn select-cropper-btn">
            <input type="file" accept="image/jpg,image/jpeg,image/png,image/gif" name="file" @change="selectImg">
            {{ $t('table.selectImage') }}
          </label>
