<template>
  <div ref="upload">
    <ImgCutter @cutDown="cutDown" :cutWidth="uploadOption.width" :cutHeight="uploadOption.height">
      <template slot="open">
        <div class="upload">
          <div class="upload_avatar">
             <img v-if="picture" :src="dataURL" alt="avatar" :style="{width:uploadOption.innerWidth+'px',height:uploadOption.innerHeight+'px'}" />
            <a-icon :type="loading ? 'loading' : 'plus'" v-else class="upload_icon"/>
          </div>
          ...
          <span class="remark">
            ...
            <p>支持尺寸：{{`${uploadOption.width} * ${uploadOption.height}`}}</p>
          </span>
        </div>
      </template>
    </ImgCutter>
...
export default {
        ...
        reader.onload = (e) => {
          ...
          image.onload = function () {
            ...
            if (w / h == ratio) {
              ...
              _this.$antmessage.error(`图片尺寸错误，只能上传${width}x${height}横纵比一致的图片`);
              reject(false)
            }
          }
        }
  ...
  mounted() {
    this.$nextTick(function () {
      this.$refs.upload.style.setProperty("--width", this.uploadOption.innerWidth + 'px')
      this.$refs.upload.style.setProperty("--height", this.uploadOption.innerHeight + 'px')
    })
  },
}
</script>
