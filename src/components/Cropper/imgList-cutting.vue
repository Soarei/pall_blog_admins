...
<template>
  <div class="imgListOut imgListOut2">
    ...
    <!-- 添加素材库图片 -->
   <div v-if="false">
     <el-button class="btnSelectImg" @click.stop="selectMaterial()" type="info" v-if="isShowKu" plain :disabled="goodsRowSupplyCode? true:false">{{ $t("goods.uploadWayLibrary") }}</el-button>
    <el-upload v-show="false" ref="upload" action class="upload-demo" accept="image/*" :multiple="type == 'goodsThumb' ? false : true" :auto-upload="false" :on-change="uploadChange" :on-remove="uploadRemove" list-type="picture">
      <el-button ref="uploadBtn" size="small" type="primary">{{
        $t("shop.clickToUpload")
      }}</el-button>
      <div slot="tip" class="el-upload__tip">
        {{ $t("payment.iconStyleMessage") }}
      </div>
    </el-upload>
   </div>
    <!-- 从素材库添加图片弹框 -->
...
export default {
  ...
  methods: {
    ...
    selectCropper() {
      ...
      this.$emit("getCuttingData",{ type: this.type, indexKey: this.indexKey, indexTab: this.indexTab })
      ...
      if (file) {
        ...
        reader.onloadend = function () {
          ...
          oImg.onload = function () {
            ...
            if (_this.type == "goodsDetailImage" && (fixgoodsImg.width != imgWidth || fixgoodsImg.maxheight < imgHeight)) {
