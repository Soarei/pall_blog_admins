    ...
    <div class="app-container">
      ...
      <div class="search">
        <div class="serachItem">
          <a-input placeholder="用户账号或用户昵称" v-model="searchForm.user_account"></a-input>
        </div>
      ...
      <a-table :columns="columns" :data-source="data" :scroll="{ x: 1000 }" rowKey="user_id" :pagination="pagination">
        <template slot="user_avatar" slot-scope="text,record">
          <img :src="record.user_avatar" alt="" srcset="" style="width:50px;height:50px;">
        </template>
        <template slot="roles" slot-scope="text,record">
          <a-tag v-for="(item,index) in record.roles" :key="index" :color="item | formatRoles">{{item}}</a-tag>
        </template>
        ...
        <template slot="action" slot-scope="text,record">
          ...
          <a style="margin-left:5px;" @click="frozenCloumn(record)">{{ record.status === 0 ? '冻结':'解冻' }}</a>
      ...
      <!-- 分配角色 -->
      <a-modal v-model="roleVisable" title="分配角色" ok-text="确认" cancel-text="取消" @ok="submitFormRole">
...
<script>
import { getAdminUser, updateAdminStatus, getRoles, getUserRolesIds, updateRolesAuth } from "@/api/menus/roles"
